...
%% EXPERIMENT DESIGN (task-event, not block design)
%-------------------------------------------------------------------------------
% BLOCKS
...
TC_event_same_FLAG = 1;  % 1=event timing will be the same for all subjects

% event probabilities (0.6 standards, 0.075 targets and novels, 0.05 CSF spikes)
TC_event_prob = [0.5, 0.2, 0.3, 0];  % an 8:1:1 ratio 
...
TC_event_amp([comp_DMN],                         1) =-0.5; % negative weak
...
TC_event_amp([comp_DMN],                         2) =-0.5; % negative weak
...
TC_event_amp([comp_DMN],                         3) =-0.7; % negative weak
